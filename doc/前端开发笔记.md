...
- 项目根目录执行以下命令
  ...
  - 注意：官方推荐npm安装方式也可以，但是配置需要自己手动配置，vue add不需要手动配置
...
http
 ...
 server {
        listen       443 ssl;
        server_name  120.55.77.129;
        
				#填写证书文件路径
        ssl_certificate  "/www/server/nginx/conf/cert/mujiabinbin.top.crt";
        #填写证书私钥文件路径 
