...
- 返回结果
  ...
  - 可以用`then()`接收返回对象数据`res.data`，`res`为整个返回对象
    ...
    getstudata(){
                 const data=axios({
                            url:'http://'
                            method:'GET'//get为默认method，可以省略不写
                            params：{
                                     _id=1,
                                    }).then((res)=>{
...
- 渲染
  ...
  - 在父路由的vue上通过<router-view>可以渲染所有该父路由下子路由，但是需要<router-link>逐一设置路由链接，点击后在子路由所属父路由的组件中的<router-view>显示相应路由组件

  - 注意，<router-view>默认显示嵌套路由需要单独通过`redirect`设置
...
- 跳转
  ...
  ```vue
  <dd><router-link to='stuadd'>新增学生</router-link></dd>
            <dd><router-link to='stulist'>学生列表</router-link></dd>
  ```
...
- 每一个路由对象，除了`path`，`name`等信息，`VueRouter`还提供了`meta`来记录路由一些额外的信息
  ...
  - 通过`this.$route`可以获取当前路由信息对象，从中拿到`meta`属性及属性值，再进行判断或使用
    ...
    - 例如判断哪些组件保持<keep-alive>缓存,此例子为vue2写法，vue3不生效
...
- 导航：路由正在发生改变，即一个路由跳到另一个路由

- 守卫：在这个过程中设置守卫来拦截这个跳转来做一些验证，决定是否进行此次跳转或跳转其他页，一般用于登陆等需要验证的场景
...
- 分类：
  ...
  - 全局守卫：作用于项目全部路由
    ...
    - 前置守卫：进入路由
      ...
      - `router/index.js`中创建router实例代码后添加
      ...
      - `to`：要跳转的路由信息对象，即：$route

      - `from`：来源的路由信息对象，也是$route

      - `next`：通过函数，表示验证通过调用此函数进入跳转路由页面
  ...
  - 路由独享守卫：作用于当前路由，写在哪个路由，就作用于这个路由

    - 前置守卫：进入路由`beforeEnter`
  ...
  - 组件内守卫：作用于与该组件相关的路由
    ...
    - 后置守卫：离开路由`beforeRouteLeave`

      - 在组件页面`export default`中添加，与`data`,`methods`等同级
        ...
        - `Object.keys(obj)`：obj对象的属性名拉出来组成一个数组，如果该数组length等于0，说明obj是空数组，该方法判断某个对象是否为空

### Vuex-状态机



- Vuex状态管理模式，vue的官方数据管理仓库，用于管理vue中的公共数据及操作数据的方法
...
- 使用：
  ...
  - 下载Vuex包并安装
    ...
    - 此命令可以下载并自动生成配置代码，不需要自己手写

      

  - `src`目录下`store`文件下`index.js`，配置该文件
    ...
    - `state`:状态，实际指项目中的数据，用来保存项目中的公共数据，类似于组件中的data

    - `getters`：用来保存从state数据身上得到的新数据，类似组件中的computed

    - `mutations`：用来保存所有**同步**修改state的方法，是唯一**修改**state的途径

    - `actions`：用来保存所有异步的方法

    - ` modules`：用来将仓库划分成多个模块
  ...
  - `main`文件配置Vuex
    ...
    ```vue
    引入仓库实例对象，完整写法：import store from './store/index.js'
    import store from './store' 
    
    //将实例对象挂载到app上，之后可以在app中通过this.$store获取此实例对象
...
- 数据(`state`)
  ...
  - 组件
    ...
    - 直接在html通过 `$store.state`引用

    ```vue
    <h1>计数器{{ $store.state.num }}</h1>
    ```

    - 通过`computed`接收后使用

      - 注意不可以用`data`接收，因为后期修改数据时用`data`接收数据不会改变

      ```vue
       <h1>计数器computednum{{ computednum }}</h1>
...
- 修改数据(`mutations`)
  ...
  - index.js
    ...
    - Vuex方法建议全部大写方便组件使用时识别

    - 默认接受第一个`state`参数获取定义在Vuex中的数据
    - 如需要第二个参数从组件获取，推荐使用`payload`关键字，但也可自己命名
    - 只有两个参数，不可以再加第三个参数
  ...
  - 组件
    ...
    - 通过`$store.commit('方法名'，参数)`调用并传参数
  
      ```vue
      <button @click="$store.commit('DECRECEMENT')">-1</button>
       ...
       watch:{
              inputnum(){
                  this.$store.commit('INPUTNUM',this.inputnum)
              }
          },
      ```
...
- 使用：
  ...
  - Index.js
    ...
    - 公共异步方法名称一般在方法名后加Async表明
    - 发送请求不再用`this.$store`，因为当前不在组件内，所以`this`不是指向vue，需要先引入请求模块
    - 得到返回数据可以保存在`state`中（公用数据），也可以直接`return`（返回值为状态码等场景）
      - return的值是一个promise对象，因此在组件中也需要通过`await`来接收
    - 公用数据赋值不能通过`actions`，只能通过`mutations`中设置方法通过参数作为传递
      - actions下面方法默认第一个参数是`context`，代表整个仓库对象，所以调用mutations方法语句为 context.commit('GETCLADATA',data.data)
      - 第二个参数`payload`与`mutations`一样，接收外部调用时传入的参数
  ...
  - 组件
    ...
    - 用`dispatch`调用actions方法,语句为`this.$store.dispatch('getclalistAsync')`
    - 通过computed中设置接收仓库响应数据
...
- 使用
  ...
  - 组件
    ...
    - 通过`this.$store.getters`直接使用
    - 也可以通过`computed`计算属性接收

    ```vue
    <h1>计数器{{ $store.getters.sum }}</h1>
...
- Vuex提供了四个辅助函数
  - `mapState`：在组件中获取state的数据
  - `mapGetters`：在组件中获取getters的数据
  - `mapMutations`：在组件中获取mutations的方法
  - `mapActions`：在组件中获取actions的方法
...
- 使用
  ...
  - 组件中引入辅助函数
    ...
    import { mapActions:xxxActions, mapMutations:xxxMutations,...} from 'vuex'；
    ```
  
    - 当一个组件需要引入多个模块的辅助函数时，需要区分不同的mapMutations，mapActions:。。。
  
  - 组件调用方法
  
    - 在methods中通过`...mapActions(['addclaAsync'])`引入仓库中的actions方法，`mapActions(['addclaAsync']`得到一个对象，通过扩展运算符`...`将得到的对象中的方法放到methods中成为为组件自己的方法，就可以通过`this.方法名`调用
      - methods还可以通过`...mapMutations(['addclaAsync'])`引入仓库中的mutations方法
    - 通过`this.方法`调用并传参数
      - 参数仓库通过payload接收
  
    ```vue
    methods: {
            ...mapActions(['addclaAsync','editclaAsync',...]),
             addcla(claadd){
                this.addclaAsync(this.claadd)
            }
        },
    ```
    ...
    - return代码案例
      ...
      - 在仓库中只做数据的处理和流转，处理完成后的操作尽量不要在仓库中做，要在组件中去做
      ...
      - 组件中调用不同模块辅助函数中的方法重名，通过对象定义新键（xxx）解决
      
        ```vue
         ...mapActions({xxx:'addclaAsync'，})
        ```
      
        
  
  - `mapState`与`mapGetters`在组件中的`computed`计算属性中引入，相当于计算属性中自己的属性
  
    - 之后一样可以通过`this.属性名`在组件中使用

     ```vue
     computed:{
         ...mapGetters(['sum']),
     		...mapState(['num','cladata']),
         stulist() {
           return this.cladata;
         },
     
     ...mapState({stylist:"studata",total: "totaldata", pages:"pagesdata", stupresrc:"stupresrc"}),
     //不用数组及return也可以通过对象设置变量接收
        },
     ```
...
- `store`目录下新建`modules`目录 ，里面存放不同模块的js文件

  - 每个js文件直接暴露各自的`state`,`mutations`...属性

    - 每个模块js文件都要加上`namespaced:true`属性，以保证每个js文件的`state`,`mutations`...属性下的属性和方法不冲突
      - 在`store`对象中加了`namespaced:true`属性的方法前面会带上所属模块路径，数据路径也会加上所属模块
...
- 非辅助函数调用
  ...
  - 方法路径要准确，加上所属模块
    ...
    ```vue
    this.$store.dispatch('classes/getclalistAsync')
    ```

  - 数据调用也需要在前面加上`classes.`

  ```vue
  
  this.$store.state.classes.cladata
  ```
...
- 项目根目录执行以下命令
  ...
  - 在vue3下add安装后最后一步css部分node sass会报错，导致项目启动报错，不使用此方式

  ```bash
  sudo vue add element
  ```

  - 注意：官方推荐npm安装方式也可以，但是配置需要自己手动配置，vue add不需要手动配置，vue3启动需要额外安装`element-plus`
...
- elementUI组件特有标签编译后以类`class`的形式被渲染，如<el-container>，<el-header>。。。所以在写style-css时需要按照类的css形式写
  ...
  - 如果页面存在多个<el-container>，设置不同的css，还是要在<el-container>标签中添加`class`
...
- 每个element标签都有特定属性，如 <el-aside width="200px">中`width`就是特定属性，编译后浏览器会以内连样式渲染
  - 所以在style-css中修改特定属性样式不会生效，需要在属性中设置，因为内连样式权重高于外部样式
  - 如果想在style-css中修改特定属性，需要根据css中更改权重的方式更改，如id权重高于class
...
- 真实项目中导航数据一般从服务端获取，前端采用循环`v-for`方式渲染，导航层级不同（如有的导航只有一级导航，有的包含二级或更多级）通过`v-if`判断
  ...
  - `router`：是否启用 `vue-router` 模式。 启用该模式会在激活导航时以 index 作为 path 进行路由跳转 使用 `default-active` 来设置加载时的激活项。
    - 激活项通过`$route.path`动态获取并匹配index的路由路径

#### 表格



- `el-table-column`直接渲染

  - `prop`获取该列数据源，数据源为数组类型
    - 若数据源为username.xxx，可以直接使用prop="username.xxx"，无需额外处理
  - `label`为该列名称
...
- `el-table-column`插槽渲染（自定义表格）

  - 当`prop`数据需要其他标签渲染时，通过声明`#default="scope"`设置插槽，设置后`scope.row`获取当前行的数据，所以`scope.row.stusrc`为该行中`stusrc`列的数据

    - 注意：Vue3匹配的`element-plus`使用`#default="scope"，`不再使用Vue2匹配的elementUI中的slot-scope="scope"
    - 若内容无需使用数据，只需声明#default即可，带上scope反而会报错
    ...
    //内容无需使用数据，只需声明#default即可，带上scope会报错
...
- 样式穿透只作用于局部样式，即<style>中添加了`scoped`属性
...
-  handleSizeChange(val)中的形参`val`会自动获得选中的每页显示条数
- handleCurrentChange(val)中的形参`val`会自动获得当前页数据
- `watch`侦听器一定要加，保证pagedata数据变化及时更新请求
...
- `res`接收上传成功返回的数据，服务端一般包括状态码，图片地址或图片名
  - 若返回图片名，则需要自己拼出完整的图片地址，`:src="imageUrl"`去掉
  - 若返回图片地址，可以通过`<img v-if="imageUrl" :src="imageUrl" class="avatar">`接收，其中imageUrl可以根据自己项目需要设置，如图片的属性名，而`:src`为固定图片地址，不可更改名称
...
- 前置准备：http目录创建登陆请求并在请求汇总api文件写入
...
- 路由页-首页（/home）添加导航守卫`beforeEnter`

  - 拿到本地存储token发给服务端接口

  - 因为路由文件不是组件，不能直接调用http-api请求模块，需要引入后使用
    ...
    - 注意：发送token时必须加上`bearer `，**bearer加上一个空格**

      ```js
      //http 用户登陆请求文件-验证token函数
      getuserinfo(){
              return axios({
                  url:"/users/islogin",
                  headers:{
      ...
      - 全局axios请求统一加token
        ...
        - 设置axios拦截器-http目录下新建axios.js

        - main.js引入拦截器`import '@/http/axios';`
    ...
    - 若本地无存储token提示错误并跳转到登陆页

      -  因为路由文件不是组件，不能直接使用`element-plus`内置组件，需要单独引入
    ...
    - 若有token判断token是否有效
      ...
      - 有效，根据返回的数据将用户名解析并存储状态机，供其他组件调用

        - 因为路由文件不是组件，不能调用store内函数及数据，所以需要引入状态机才能使用
    ...
    ```js
    {
        ...
        beforeEnter: async(to, from, next) => {
          ...
          if(token){
            ...
            if(data.status==1){
              next();
              store.commit('GETUSERINFO',data.data)//若不是在主仓库，需要加上“模块名/函数名”
            }
            console.log(data);
...
- 每一个组件都有一个属性`filters`
  ...
  - `val`：接收 `scope.row.userclaname` 作为对象，然后在定义函数`isclaname`中写逻辑处理数据
...
- 自定义节点最主要作用是**可以拿到页面原生节点并通过原生js语法对节点进行操作**

  

#### 局部

- 每一个组件都有一个`directives`属性用来设定自定义属性
...
- vue给每个组件提供了一个`mixins`属性，属性值是一个数组，用来接收多个混入对象
  ...
  - 混入对象就是用来提取多个组件中相同的js逻辑代码，混入对象本身拥有组件一样的属性，包括`data`,`methods`声明周期等

  - 例如：将学生列表的分页数据及操作提取到mixins

    - src目录下新建mixins文件夹，文件夹下新建js文件pagedatamix
    ...
    - 在stu组件引入该文件并在`mixins`使用
      ...
      - 注意：stu组件仍旧可以有自己的`data`，`methods`。。。只是定义时候属性或方法名不要与mixins中定义的重复即可
...
- 创建实例
  ...
  - 获取html节点并创建实例
    ...
    - 获取节点需要在`mouted`中获取，因为此时页面节点已经加载完成
  ...
  export default {
    mounted() {
      //const main=this.$refs.stuchartsref;
  ...
  - 图表渲染
    ...
    - `setOption`是echarts函数，不具备vue相应式效果，所以当数据为变量时需要通过`watch`来侦听数据变量，以控制数据图表响应式变化
      ...
      - 获取数据保存在`data`选项，而对原始数据进行处理后得到的图表所需数据通过`computed`选项进行处理

### 虚拟节点



- 虚拟节点（Virtual DOM）就是一个以js对象为基础的节点树，即数对象中包含了div属性，h1属性。。。

- 渲染过程

  - 虚拟节点存储在内存中，当页面数据发生变化，虚拟节点会对比与之前虚拟节点的变化并更新变化的地方，即尽可能在虚拟节点中操作，节省效率，性能优化，减少重绘和回流

  ![90a4dc6dee742586321212c44b4199a7](/Users/mujiabin/Documents/VsCode/practise/elestusys/public/img/:Users:mujiabin:Downloads:90a4dc6dee742586321212c44b4199a7.png)
...
- 源码解析
  ...
  - 数据拦截
    ...
    - `Object.defineProperty(object name,object property,{get(){},set(){}})`
      - object name：拦截数据对象名
      - object property：拦截数据对象属性名
      - get(){}：数据被引用触发
      - set(){}：数据被修改触发
  ...
  - watcher（观察者）与dep（订阅者）

    - watcher：每条数据和每个节点关系，如果一条数据被十个节点使用，会产生是个对应watcher
    ...
    - dep：每一个数据都有一个dep，每个dep管理每个数据对应的所有watcher
    ...
    ![截屏2023-04-25 22.57.46](/Users/mujiabin/Documents/VsCode/practise/vueSourceCode/img/:Users:mujiabin:Desktop:截屏2023-04-25 22.57.46.png)
...
- 检查node.js是否安装，要求版本12.0以上



#### npm

- 检查npm版本，根据版本不同使用不同创建项目版本

  - my-vue-app为项目默认名，可以修改为自己项目的名字

  ```bash
  sudo npm create vite@latest my-vue-app -- --template vue
  ```
...
- 按需引入，需要什么都要引入，如`reactive`，`ref`，`computed`，侦听器，生命周期。。。,不会像Vue2将全部选项卡属性全部引入
...
- 父组件引入子组件后不需要<components>注册即可使用，只需要引入和渲染

- 模板<template>可以包含多个跟节点，即不需要通过<div>包裹

- 子组件接收数据改为`defineProps`

- v-for 中的:key可以放到<template>上

- 数据

  - `ref`可以设置数据，一般用于单个数据保存

  - 常用类似Vue2中data的是`reactive`，为一个对象
...
- 全局变量
  ...
  - 不同于vue2，`main.js`中虽然可以设置全局变量，但设置后组件中调用不可直接用`this.变量`
  ...
  - 解决办法2:引入`getCurrentInstance`和`proxy`
    ...
    const apitest = async () => {
    		const data = await proxy.api.indexapi.testapi()
    		console.log(data);
    	}
    ```
...
- 计算属性
  ...
  - 使用计算属性需要引入,使用时通过箭头函数为参数，返回一个值作为`computed`结果，并通过变量接收
...
- 侦听器`watch`
  ...
  - 使用侦听器需要引入
  - 不需要变量接收
  - 包含两个箭头函数，第一个函数return侦听的数据，第二个函数为侦听数据发生变化要执行语句
...
- `watchEffect`
  ...
  - 无需指定侦听数据，只要`watchEffect`内的数据变化都会侦听到并通过并执行相应语句
...
- 生命周期函数
  ...
  - vue3中没有`beforeCreated`和`Created`函数，`Mouted`变为`onMounted`，其余生命周期函数都加**on**
...
- 项目根目录执行以下命令，@next表示最新

```bash
sudo npm i vue-router@next vuex@next
```
...
- 在src/下创建router/index.js作为路由配置文件
...
- 路由跳转
  ...
  - <routerlink>照常使用
  - js跳转需要引入useRouter后调用一下通过一个变量接收，即拿到router对象而后使用
    - 同理route对象也是引入useRoute后通过变量接收
...
- 在src/下创建store/index.js作为状态机的配置文件
...
- 组件获取状态机数据方法
  ...
  - 引入useStore后调用并通过一个变量接收，而后通过`变量.state`，`变量.commit('方法名'，‘参数’)`，`变量.dispatch('方法名'，‘参数’)`等方法拿到数据或方法
    - 注意模块化后调用数据或方法需要在前面加上模块名，详见前面模块化笔记
  - 辅助函数在set up模式下使用不方便，直接用上边的方法单独调用即可
...
- 详见elestusys，但未用js setup方式，无新知识，只是不用设置全局变量以及状态机辅助函数

  ```js
  app.config.globalProperties.api = api;//不用设置
  ```
...
- AppID(小程序ID)：wxdeca28210fef99bf
...
-  页面路径配置文件，可以在此配置新增页面，保存后pages目录下自动生成四个页面文件
  - 数组第一个页面路径为用户首次访问时显示的页面，可以根据自己需要配置首页
  - 开发时可以设置`添加编译模式`，设置开发时的默认首页
  - 数组最后一个页面不加逗号，否则报错
  - window可以设置全局样式，导航栏颜色等，颜色不能英文，需要十六进制表示
  - 导航栏文字颜色只能是黑色或白色
  - `navigationBarTitleText`，小程序名称，根据自己项目名称更改
  - json文件中需要加引号的部分必须是双引号
...
- 对象，包括公共属性及数组list（tab内容）
- tabBar包含的tab页面会出现tabBar，不包含的页面不出现
...
"tabBar": {
    ...
    "list": [{
      ...
      "text": "首页",
      "iconPath": "./img/zhuye-2.png",
      "selectedIconPath": "./img/zhuye.png"
    },
    {
...
- 每个页面文件夹中的json文件，配置内容与全局属性差不多一致，优先级高于全局属性

  - 例如单个页面配置了导航标题（navigationBarTitleText），此标题就会应用于这个页面，不应用全局配置导航标题
...
- 滑块不止用于轮播图，凡是需要手动滑的都可以使用

  - `swiper`高度需要设置一般为100vh，`swiper-item`高度默认100%

    - 但考虑到`swiper`有时不是页面全部的组件，还需要将其他组件高度减去

      ```css
      height:clac(100vh - 20rpx)
      ```

      - 注意，减号两边需要空格

```js
<swiper class="home-swiper" autoplay="true" interval="3000" duration="2000" circular indicator-dots>
  <swiper-item>
    <image src="/img/banner/banner1.png" alt=""/>
  </swiper-item>
  <swiper-item>
    <image src="/img/banner/banner2.png" alt=""/>
  </swiper-item>
  <swiper-item>
    <image src="/img/banner/banner3.png" alt=""/>
  </swiper-item>
</swiper>
```
...
- 小程序尺寸单位，不管手机屏幕尺寸，宽度都会为750rpx，开发时根据具体元素尺寸做相应换算
...
data: {
    swiperdata:[
      {id:1,path:"/img/banner/banner1.png",show:true},
      {id:2,path:"/img/banner/banner2.png",show:true},
      {id:3,path:"/img/banner/banner3.png",show:true}
    ]
  },
```
...
- wxml
  ...
  - 动态数据无论属性还是显示都用双大括号表示`{{}}`
  - 循环不需要item in array，直接`wx:for="{{array}}`，下面默认item和index表示每一项和下标
  - 字符串拼接直接拼即可，遇到变量用双大括号即可 ，例如<image src="/img/banner/{{item.path}}" alt=""/>
  - `wx:key="id"`特殊，虽然是动态，但不加双大括号，而且直接写`属性名`即可，不用`item.属性`
...
- 步骤：
  ...
  - components目录下新建文件夹，名称自取

  - 目录右键新建`component`，名称与目录名称一致，会自动生成四个文件
  ...
  - 引用组件
    ...
    - 页面json页面引入组件
      ...
      ```js
       "usingComponents": {
          "home-swiper":"/components/home-swiper/home-swiper"
        }
      ```
    ...
    - 页面wxml通过组件标签显示组件
      ...
      - 数据传递 
        ...
        - {{swiperdata}}定义在页面js文件中的`data`属性
...
-  发送请求`wx.request({})`，请求函数放在与`data`同等级，而后在生命周期中调用
  - 组件中函数还是写在`methods`选项中
  - 请求上线时需要保证请求连接为合法域名，在小程序管理-开发管理中设置且请求协议为https，开发中可以设置成不校验，但上线不行
-  赋值：`this.setData({变量:res.data...})`
  - 注意，this指向应为请求函数getcladata，而不是success函数，所以success必须用到箭头函数，保证this指向
  - 直接赋值可以改变数据，但页面不会更新，页面更新数据必须this.setData方法
  - 可以通过this.setData给多条数据赋值
  - 使用data选项卡定义的数据，在`this.变量`中加上一个data，这个是小程序的特殊设定

```js
getcladata(){
    wx.request({
      url: 'http://120.55.77.129:3000/class/get',
      method:"GET",
      success:(res)=>{
        console.log(res);
        this.setData({
...
- 小程序生命周期是在`app.js`中调用的，App(Object)函数用来注册一个小程序，接受一个Object参数，指定其生命周期回调

  | 生命周期       | 参数 | 描述                                              |
  | -------------- | ---- | ------------------------------------------------- |
  | onLaunch       | 无   | 小程序初始化完成时执行（全局只触发一次）          |
  | onShow         | 无   | 小程序启动，或后台切换到前台时执行                |
  | onHide         | 无   | 小程序隐藏，即从前台切换到后台时执行              |
  | onError        | 无   | 小程序脚本错误，或api调用失效触发，会带上错误信息 |
  | onPageNotFound | 无   | 组件实例从页面节点数上移除时执行                  |
...
- 页面生命周期定义在页面`page`构造器第一级参数中

  | 生命周期 | 参数    | 描述                                                      |
  | -------- | ------- | --------------------------------------------------------- |
  | onLoad   | options | 页面加载时执行                                            |
  | onReady  | 无      | 页面初次渲染完成时执行                                    |
  | onShow   | 无      | 页面显示时执行，如从a页面跳转到b页面，b页面显示           |
  | onHide   | 无      | 页面隐藏时执行，如从a页面跳转到b页面，a页面隐藏，可以回退 |
  | onUnload | 无      | 页面卸载时执行，不可回退                                  |
...
- 组生命周期定义在组件`component`构造器第一级参数中

- 基础版本2.2.3起，组件生命周期也在`lifetimes`字段内进行声明（推荐方式，优先级最高）

  | 生命周期 | 参数         | 描述                               | 最低版本 |
  | -------- | ------------ | ---------------------------------- | -------- |
  | created  | 无           | 组件实例刚创建时执行               | 1.6.3    |
  | attached | 无           | 组件实例进入页面节点树执行         | 1.6.3    |
  | ready    | 无           | 组件在视图层布局完成后执行         | 1.6.3    |
  | moved    | 无           | 组件实例被移动到节点树另一位置执行 | 1.6.3    |
  | detached | 无           | 组件实例被从页面节点树移除执行     | 1.6.3    |
  | error    | Object Error | 每当组件方法抛出错误时执行         | 2.4.1    |

  ```js
  // components/sliderbanner/sliderbanner.js
  Component({
    //2.2.3以下
    created(){
    ...
    data: {
      ...
      //   {id:1,path:"/img/banner/banner1.png",show:true},
      //   {id:2,path:"/img/banner/banner2.png",show:true},
      //   {id:3,path:"/img/banner/banner3.png",show:false}
      // ]
    },
...
- 还有一些生命周期，他们并非与组件有很强的关联，但有时组件需要获知，以便组件内部处理，这样的生命周期成为组件在页面的生命周期，在pageLifetimes中定义。

  | 生命周期 | 参数        | 描述                       | 最低版本 |
  | -------- | ----------- | -------------------------- | -------- |
  | show     | 无          | 组件所在页面被展示时执行   | 2.2.3    |
  | hide     | 无          | 组件所在页面被隐藏时执行   | 2.2.3    |
  | risize   | Object Size | 组件所在页面尺寸变化时执行 | 2.4.0    |
...
- 高度一定，当内容超出组件高度时产生滑动
  ...
  - enable-flex=“true”,需要设置此项以保证盒子可以设置成弹性盒子
...
<view class="cat-container">
  <scroll-view class="cat-nav" scroll-y>
    <block  wx:for="{{catsdata}}" wx:key="catid">
      <view wx:if="{{item.catid==currentid}}" class="cat-nav-active">
      <text>{{item.catname}}</text>
    </view>
    <view wx:else class="cat-nav-view">
      <text>{{item.catname}}</text>
    </view>
    </block>
    
  </scroll-view>
  <scroll-view class="cat-content" scroll-y>
    <view wx:for="{{10}}">
      <text>电视</text>
      <view wx:for="{{20}}">
        <text>索尼</text>
...
- `bind`：普通事件绑定，不会阻止事件冒泡，常用

  ```html
  <button bindtap="事件处理函数">
...
- 自定义属性`data-`
  ...
  - 获取参数
    ...
    ```html
    <view bindtap="changeactive" data-id="{{item.catid}}" 
    ```
...
- `mark`：2.7版本后可用
  ...
  - 获取参数
    ...
    ```html
    <view bindtap="changeactive" mark:id="{{item.catid}}" 
    ```
...
- <navigator>
  ...
  <navigator url="/pages/products/products" wx:for="{{10}}">
      <view >
        <text>电视</text>
        <view class="seccatdiv">
          <view class="seccat" wx:for="{{20}}">
          <image src="/img/banner/banner1.png"/>
          <text>索尼</text>
        </view>
        </view>
...
| 方法                             | 说明                                                 |
| -------------------------------- | ---------------------------------------------------- |
| **wx.switchTab(Object object)**  | 跳转到tabbar页，并关闭非tabbar所有页                 |
| wx.reLaunch(Object object)       | 关闭所有页，并打开应用内某个页面                     |
| wx.redirectTo(Object object)     | 关闭当前页，跳转到应用内某个页，但不能是tabbar页     |
| **wx.navigateTo(Object object)** | 保留当前页，跳转到应用内某个页面，但不能是tabbar页面 |
| wx.navigateBack(Object object)   | 关闭当前页，返回上一页或多级页                       |
...
- 传递：页面间跳转时，通过url已经进行传参

  ```html
  <navigator url="/xxx/xxx/xxx/?id=1">
    <navigator url="/xxx/xxx/xxx/?id=1&name=xxx">
      
      <navigator url="/pages/products/products?cid={{item.id}}" 
  ```

  ```js
  wx.navigateTo({url:"/xxx/xxx/xxx/id=1"})
  ```

  

- 接收：在页面生命周期函数`onLoad(options)`中通过options接收

  - 发送网络请求参数通过data属性配置

  ```js
  console.log(options)//输出{id:“1”，name：“xxx”}
  onLoad(options) {
  getcladata(options){
      wx.request({
        url: 'http://120.55.77.129:3000/class/get',
...
- 小程序中提供了方法`wx.getUserProfile()`来获取用户信息，老版本为wx.getUserinfo()，此方法必须是用户点击事件触发，不可以在生命周期中使用，即用户进入页面调用

- 但最新基础包已经回收此接口

  ![0](/Users/mujiabin/Documents/VsCode/practise/doc/img/:Users:mujiabin:Downloads:0.png)
...
- 代码
  ...
  ```html
  <button bindtap="getuserinfo">用户登陆</button>
  ```
  ...
  getuserinfo(){
      wx.getUserProfile({
        ...
        success:(result)=>{
          ...
          //将用户信息保存只全局变量app.js,globalData
          getApp().globalData.userInfo=result.userInfo
          //此处可以直接=赋值，因为不是data中的数据赋值
        },
        fail:(res)=>{
...
- 最新方法
  - https://www.jb51.net/article/283142.htm

#### 小程序登陆



![api-login.2fcc9f35](/Users/mujiabin/Documents/VsCode/practise/doc/img/:Users:mujiabin:Downloads:api-login.2fcc9f35.png)
...
- 登陆
  ...
  ```js
  //html
  <button bindtap="getcode">getcode</button>
  //获取code
  getcode(){
      wx.login({
        success: (res) => {
    ...
    //发送code到服务端，等待返回token并将token存储到本地存储
    gettoken(code){
      wx.request({
        url: 'url',
        method:'POST',
        ...
        success:(res)=>{
          console.log(res);
          wx.setStorage({//异步函数，wx.setStorageSync('key', data)同步函数
            key:'token',
            data:res.data.token
          });
          wx.navigateBack({
            delta:1//回退一个页面，在页面onshow中拿userinfo
          })
        }
      })
    },
...
- 个人中心
  ...
  ```js
  onShow() {//必须在onShow中执行判断，即页面监听；onload和onready都是程序开启或页面销毁重新进入时才会触发
  	//判断用户信息是否为空，为空显示未登录状态，不为空显示用户信息及登陆状态
    },
  ```
...
- 与小程序创建一致，只是开发模式选择微信云开发
- 小程序编辑器上方点击云开发图标
  - 创建数据库及记录
    - 底层为mongodb
    - 设置权限-自定义规则，read与write都设置为true
    ...
    onLaunch: function () {
      if (!wx.cloud) {
        ...
        wx.cloud.init({
          ...
          //   如不填则使用默认环境（第一个创建的环境）
          env: 'cloud-test-3gs47s2yc8405389',
          traceUser: true,
        });
      }
...
- 云开发项目中，专门有一个目录（cloudfunctions）存放云函数

  - 新建云函数：目录右键，选择新建nodejs云函数

  - 在新建云函数文件夹中编辑index.js
  ...
  - 上传部署云函数：云函数文件夹，右键，上传并部署（云端安装依赖）
  ...
  - 调用云函数
    ...
    - 先写自己的方法，然后在自己方法调用
      ...
      - 通过data向云函数传值，云函数通过event接收

    

- 云函数调试
  - 云函数文件夹-右键-云函数本地调试
  - 新开页面右侧勾选开启本地调试，按提示安装环境插件
...
- `pages.json`，与小程序类似，配置pages，tabbar，window。。。
  ...
  ```js
  {
  	...
  	"tabBar": {
  		...
  		"list": [{
  			"pagePath": "pages/index/index",
  			"iconPath": "static/image/home-block.png",
  			"selectedIconPath": "static/image/home-block-2.png",
  			"text": "首页"
  		}, {
  			"pagePath": "pages/list/list",
  			"iconPath": "static/image/sort.png",
  			"selectedIconPath": "static/image/sort-2.png",
  			"text": "列表"
  		}]
  	},
  	"uniIdRouter": {}
...
- `static`文件夹，存放项目静态资源，不可更改名称，但下面可以新建文件夹

- `components`文件夹，自定义控件，components=>`控件名`文件夹=>`控件名.vue`，以此格式定义的控件在页面可以直接`<控件名></控件名>`使用
...
- 根目录下新建`api`目录，保存所有网络请求模块

  

#### uni.request封装

- api目录下新建`request.js`文件

- 因为没有axios这样的成熟异步方案，所以需要自定义请求函数`request(param)`
  - 参数`param`为形参，为发送请求调用此函数传入的对象，对象内容参照官方uni.request文档
...
- uni.request中`complete`无论是否成功接收到参数，都执行下面语句
...
- `api`目录下新建`const.js`文件

  ```js
  const BASEURL = 'http://47.98.128.191:3003/'
  export const BANNERURL = BASEURL + 'home/getBanners';
  export const MOVIEURL = BASEURL + 'movies/getMovies'
  ```
...
- api目录下新建modules目录，里面根据页面及包含的请求划分不同请求模块
...
- components——home-swiper文件夹——home-swipe.vue

  ```js
  <template>
  	<view>
  		<swiper :indicator-dots="true">
  			<swiper-item v-for="item in bannerdata" :key="item._id" v-if="item.show">
  	...
  	export default {
  		...
  		methods: {
  			async getbanner() {
  				...
  				this.bannerdata = data.result.rows
  				console.log(data);
  			}
  		}
  	}
...
- 为了解决跨段兼容性问题，可以设置某段代码在某个平台作用或不作用

- 定义的范围可以是页面，组件，js文件，json文件，静态文件（static下单独目录）
...
- ```html
  ...
  - <home-swiper>代码除了在APP-PLUS平台外都会出现
...
- 权限设置
  ...
  - manifest.json-源码视图
    ...
    - 添加permission和requiredPrivateInfos属性
...
- 扫码登录-https://lbs.qq.com

- 创建应用

- 新建key获取密钥

- 下载微信小程序JavaScriptSDK解压并放入项目中，可在根目录新建`utiles`目录存放

- 小程序开发设置添加request域名，https://apis.map.qq.com
...
- 实例化qqmap
  ...
  onload(){
    qqmapsdk=new QQMapWX({
      key:'KYKBZ-Z7VKZ-QIOX6-ZEJCC-NQT7T-QTBXZ'
    })
  }
  ```
...
- HBuilderX-工具-插件安装-插件市场
- 下载后在项目-uni_modules目录中出现秋云图表组件，符合uniapp标准，可以直接在页面标签使用
...
- SSH登陆
  ...
  - terminus
    ...
    - 密码：mujiabinbin0715
- 宝塔图形面板
  - SSH连接服务器，输入`bt`命令，根据返回选择`14`会返回登陆地址，用户名和密码
- 域名：mujiabinbin.top
  - 服务商：阿里云
- SSL：mujiabinbin.top.crt,mujiabinbin.top.key
  - 服务商：七牛
- 七牛：https://www.qiniu.com
  - 账号：18618180537
  - 密码：Mjbb0715?
  - AccessKey:GjYp-aD9ESL01dlbl5w1Dx7CsT7KxifPyWTr13Ae
  - SecretKey:tiYm4RsjZSxB2Y0uVXpchaMHin-hB1kpsIGrxHK7
  - 测试域名：rv5zn257t.hn-bkt.clouddn.com
  - 上传地址(domain)：http://upload-z2.qiniup.com
...
http
 ...
 server {
        listen       443 ssl;
        server_name  mujiabinbin.top;
        
				#填写证书文件路径
        ssl_certificate  "/www/server/nginx/conf/cert/mujiabinbin.top.pem";
        #填写证书私钥文件路径 
        ssl_certificate_key  "/www/server/nginx/conf/cert/mujiabinbin.top.key";
        ...
        location / {
            ...
            proxy_pass http://120.55.77.129:3000;
...
server {
        ...
        root  /www/wwwroot/pic-upload;
    }

server {
        listen    443 ssl   ;
        server_name  api.mujiabinbin.top;

				#填写证书文件路径
        ssl_certificate  "/www/wwwroot/https/api.mujiabinbin.top.pem";
        #填写证书私钥文件路径 
        ssl_certificate_key  "/www/wwwroot/https/api.mujiabinbin.top.key";
        ssl_session_cache    shared:SSL:1m;
        ssl_session_timeout  5m;
        ...
        ssl_prefer_server_ciphers  on;
         location / {
            ...
            proxy_set_header X-Nginx-Proxy true;
            proxy_set_header Connection "";
            proxy_pass http://mujiabinbin.top:3000;
            //http://120.55.77.129:3000 可以是域名或ip，但不能是https
            proxy_redirect default;
            #root   html;
            #index  index.html index.htm;
        }
...
server
    {
        listen 80;
        server_name mujiabinbin.top;
        return 301 https://mujiabinbin.top;
        //跳转到https
        #error_page   404   /404.html;
        include enable-php.conf;
...
- http server可以直接root配置到项目所属目录
- https server需要增加`proxy_pass http://120.55.77.129:3000`及` location / {}`其他配置，将https加域名或IP转到相应服务监听的路径及端口，如express就是端口3000
  - https未通过域名备案无法生效，因为ssl证书是绑定域名与ip的关系
  - ssl证书区分单域名证书和多域名证书，单域名证书二级子域名不可用，多域名证书可以
    - 如mujiabinbin.top需要配置一个前端访问和一个后端服务，后端服务需要单独二级域名api.mujiabinbin.top并申请ssl证书，并通过http代理指向服务及端口
    - 前端服务接口访问链接https+接口链接，不需要加端口

- phpmyadmin配置特殊，记住上面代码，端口888需要在防火墙和端口管理开通

### 项目目录

- 一般配置在：www/wwwroot/项目目录名
...
- 宝塔面板-软件商店-搜索nodejs版本管理器
- 安装后一键安装nodejs，npm，PM2
  - 注意：命令行版本一定选上已安装的nodejs，保证在SSH客户端可以使用npm，PM2等命令
...
- 下载安装：宝塔面板-数据库-mongo安装
- 外网访问：宝塔面板-软件商店-MongoDB 4.4.9
- 配置项：`bindip`原本为127.0.0.1，只允许本地(服务器)程序源访问；改为0.0.0.0，其他来源可以通过mongo协议访问mongo数据库
...
```vue
<div class="upload">
		<el-upload class="avatar-uploader" :action=domain :http-request=upqiniu :show-file-list="false"
...
- 外链默认域名需要在七牛-*自定义CDN加速域名*中设置并配置CNAME后生成七牛对应的CNAME（如：img-mujiabinbin-top-idvn3g6.qiniudns.com）
- 在域名商处添加一条二级域名（如：img.mujiabinbin.top），类型CNAME，记录并指向七牛生成的CNAME
- 注意：外链默认域名不要填写成七牛的CNAME，如：img-mujiabinbin-top-idvn3g6.qiniudns.com
...
data() {
  ...
  methods: {
    upqiniu(req) {
				...
				// 重命名要上传的文件
				const keyname = 'mujiabinbinoss' + parseInt(new Date().getTime() / 1000) + '' + '.' + filetype
				// 从后端获取上传凭证token
...
// 创建上传凭证
const accessKey = 'GjYp-aD9ESL01dlbl5w1Dx7CsT7KxifPyWTr13Ae'
const secretKey = 'tiYm4RsjZSxB2Y0uVXpchaMHin-hB1kpsIGrxHK7'
const mac = new qiniu.auth.digest.Mac(accessKey, secretKey)
const options = {
	scope: 'mujiabinbinoss',
	expires: 7200
}
...
- 编辑定时任务
  ...
  20 *  *  *  * root pm2 reload app
  ```

  - 每隔20分钟运行命令：`pm2 reload app`
