...
- 选择器：
  ...
  - ::before：
    - 在标签内容之前添加一块渲染区域，可以添加内容或样式。
    - 需要添加content使用，否则无效果。
    - 行内元素，非block，不可设置高宽，如需要转化支持宽高，需要display设置转换。
  - ::after：
    - 在标签内容之后添加一块渲染区域，可以添加内容或样式。
    - 需要添加content使用，否则无效果
    - 行内元素，非block，不可设置高宽，如需要转化支持宽高，需要display设置转换。
...
- 特点：
  - 设置绝对定位元素脱离文档流
  - 只设置绝对定位，元素是在当前位置脱离文档流，一旦设置偏移量，元素默认参考整个文档进行移动
  - 原来空间不再占用
...
- 定位父级：设置的绝对定位的元素进行移动时的参考位置，默认按照整个文档进行移动，一旦父级元素设置相对定位，绝对定位和固定定位后，参考最近定位父级别进行移动，参考最近的定位父级进行移动
